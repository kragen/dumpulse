...
enum {
  n_variables = 64,
  checksum_len = 4, timestamp_len = 2, id_len = 1, value_len = 1, /* bytes */
  entry_size = timestamp_len + id_len + value_len,
  heartbeat_magic = 0xf1,
  mod_adler = 65521
};

typedef struct {
  char table[checksum_len + n_variables * entry_size];
} dumpulse;
...
{
  char *item;
  if (entry > n_variables) return 0;
  item = p->table + checksum_len + entry_size * entry;
...
static inline uint8_t process_heartbeat(dumpulse *p, char *data)
{
  ...
  uint8_t *payload = data + checksum_len;
  uint32_t checksum = adler32(payload, timestamp_len + id_len + value_len);
...
{
  store_little_endian_u32(p->table, adler32(p->table + checksum_len,
                                            sizeof(p->table) - checksum_len));
